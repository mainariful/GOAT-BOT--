        ...
        async function create_(userID, userInfo) {
                ...
                const queue = new Promise(async function (resolve_, reject_) {
                        try {
                                ...
                                if (!userInfo) {
                                        const userInfoResponse = await api.getUserInfo(userID).catch(() => ({}));
                                        userInfo = (userInfoResponse && userInfoResponse[userID]) || null;
                                }
                                if (!userInfo) {
                                        userInfo = { name: `User ${userID}`, gender: "UNKNOWN", vanity: "" };
                                }
                                let userData = {
                                        userID,
                                        name: userInfo.name,
                ...
                return new Promise(async function (resolve, reject) {
                        taskQueue.push(async function () {
                                try {
                                        ...
                                        if (!updateInfoUser) {
                                                const userInfoResponse = await api.getUserInfo(userID).catch(() => ({}));
                                                updateInfoUser = (userInfoResponse && userInfoResponse[userID]) || null;
                                        }
                                        if (!updateInfoUser) {
                                                updateInfoUser = { name: infoUser.name || `User ${userID}`, vanity: infoUser.vanity || "", gender: infoUser.gender || "UNKNOWN" };
                                        }
