...
const { configCommands } = global.Anchestor;
                ...
                if (
                        ...
                        if (fs.existsSync(path.join(__dirname, fileName)))
                                return message.reply(getLang("alreadExist"), (err, info) => {
                                        global.Anchestor.onReaction.set(info.messageID, {
                                                commandName,
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
        ...
        const storageCommandFilesPath = global.Anchestor[folder == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"];
        ...
        try {
                ...
                const { Anchestor } = global;
                const { onFirstChat: allOnFirstChat, onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = Anchestor;
                ...
                if (!oldCommandName) {
                        if (Anchestor[setMap].get(oldCommandName)?.location != pathCommand)
                                throw new Error(`${commandType} name "${oldCommandName}" is already exist in command "${removeHomeDir(Anchestor[setMap].get(oldCommandName)?.location || "")}"`);
                }
                if (oldCommand.config.aliases) {
                        ...
                        for (const alias of oldAliases)
                                Anchestor.aliases.delete(alias);
                }
                ...
                if (configCommand.aliases) {
                        ...
                        for (const alias of aliases) {
                                ...
                                if (Anchestor.aliases.has(alias))
                                        throw new Error(`alias "${alias}" is already exist in ${commandType} "${Anchestor.aliases.get(alias)}" with file name "${removeHomeDir(Anchestor[setMap].get(Anchestor.aliases.get(alias))?.location || "")}"`);
                                Anchestor.aliases.set(alias, scriptName);
                        }
                }
                if (envGlobal) {
                ...
                Anchestor[setMap].delete(oldCommandName);
                Anchestor[setMap].set(scriptName, command);
...
function unloadScripts(folder, fileName, configCommands, getLang) {
        ...
        const { Anchestor } = global;
        const { onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = Anchestor;
        ...
        delete require.cache[require.resolve(pathCommand)];
        Anchestor[setMap].delete(commandName);
