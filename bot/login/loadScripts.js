...
const { configCommands } = Anchestor;
                ...
                for (const data of aliasesData) {
                        ...
                        for (const alias of aliases)
                                if (Anchestor.aliases.has(alias))
                                        ...
                                        Anchestor.aliases.set(alias, commandName);
                }
        }
        ...
        for (const folderModules of folders) {
                ...
                for (const file of Files) {
                        ...
                        try {
                                ...
                                if (Anchestor[setMap].has(commandName))
                                        throw new Error(`${text} "${commandName}" already exists with file "${removeHomeDir(Anchestor[setMap].get(commandName).location || "")}"`);
                                ...
                                if (aliases) {
                                        ...
                                        for (const alias of aliases) {
                                                ...
                                                if (Anchestor.aliases.has(alias))
                                                        throw new Error(`alias "${alias}" already exists in ${text} "${Anchestor.aliases.get(alias)}" with file "${removeHomeDir(Anchestor[setMap].get(Anchestor.aliases.get(alias))?.location || "")}"`);
                                ...
                                if (onChat)
                                        Anchestor.onChat.push(commandName);
                                if (onFirstChat)
                                        Anchestor.onFirstChat.push({ commandName, threadIDsChattedFirstTime: [] });
                                if (onEvent)
                                        Anchestor.onEvent.push(commandName);
                                if (onAnyEvent)
                                        Anchestor.onAnyEvent.push(commandName);
                                Anchestor[setMap].set(commandName.toLowerCase(), command);
                                commandLoadSuccess++;

                                global.Anchestor[folderModules == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"].push({
