...
exports.handleRelogin = async function() {
    try {
        ...
        if (
            !config.facebookAccount ||
            !config.facebookAccount.email ||
            !config.facebookAccount.password ||
            !config.facebookAccount["2FASecret"]
        ) {
            logger('Thiếu thông tin tài khoản trong config.json', '[ ERROR ]');
            return false;
        }
        const { email, password, "2FASecret": twoFA } = config.facebookAccount;
        ...
        const cookiePath = join(process.cwd(), 'account.txt');
        ...
        const tokensPath = join(process.cwd(),'./bot/data/tokens.json');
        let tokens = {};
        ...
        const shouldRestart = process.env.AUTOLOGIN_RESTART === 'true'
            || (global.Anchestor && global.Anchestor.config && global.Anchestor.config.autoLogin && global.Anchestor.config.autoLogin.restartAfterSuccess === true);

        if (shouldRestart) {
            logger('Đăng nhập thành công, đang khởi động lại...', '[ SUCCESS ]');
            if (global.client) {
                try { global.client.commands = new Map(); } catch (_) {}
                try { global.client.events = new Map(); } catch (_) {}
            }
            setTimeout(() => {
                process.exit(1);
            }, 1000);
        } else {
            logger('Đăng nhập thành công', '[ SUCCESS ]');
        }
        return true;
    } catch (error) {
