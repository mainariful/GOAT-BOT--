...
function isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, lang) {
        const config = global.Anchestor.config;
        const { adminBot, hideNotiMessage } = config;

        const _gBans = global.Anchestor.globalBans || {};
        if (_gBans[senderID]) {
                const { reason: _gr, time: _gt, bannedBy: _gb } = _gBans[senderID];
                message.reply(
                        `⛔ | You have been globally banned from Anchestor Bot.\n` +
                        `» Reason: ${_gr || "No reason"}\n` +
                        `» Time: ${_gt}`
                );
                return true;
        }
        ...
        const _maint = global.Anchestor.maintenance || {};
        if (_maint.enable === true && !adminBot.includes(senderID)) {
                message.reply(_maint.message || "🔧 Bot is under maintenance. Please wait.");
                return true;
        }
...
module.exports = function (api, threadModel, userModel, globalModel, usersData, threadsData, globalData) {
        return async function (event, message) {
                ...
                const { utils, client, Anchestor } = global;
                const { getPrefix, removeHomeDir, log, getTime } = utils;
                const { config, configCommands: { envGlobal, envCommands, envEvents } } = Anchestor;
...
try {
        ...
        console.log("[MENTION FIX ERROR]", e);
                ...
                async function onStart() {
                        ...
                        let command = Anchestor.commands.get(commandName) || Anchestor.commands.get(Anchestor.aliases.get(commandName));

                        const aliasesData = threadData.data.aliases || {};
                        for (const cmdName in aliasesData) {
                                if (aliasesData[cmdName].includes(commandName)) {
                                        command = Anchestor.commands.get(cmdName);
                                        break;
                                }
                        }
                ...
                async function onChat() {
                        const allOnChat = Anchestor.onChat || [];
                        const args = body ? body.split(/ +/) : [];
                        for (const key of allOnChat) {
                                const command = Anchestor.commands.get(key);
                                if (!command)
                                        continue;
                                const commandName = command.config.name;
                ...
                async function onAnyEvent() {
                        const allOnAnyEvent = Anchestor.onAnyEvent || [];
                        let args = [];
                ...
                async function onFirstChat() {
                        const allOnFirstChat = Anchestor.onFirstChat || [];
                        const args = body ? body.split(/ +/) : [];
                        ...
                        for (const itemOnFirstChat of allOnFirstChat) {
                                ...
                                if (threadIDsChattedFirstTime.includes(threadID))
                                        continue;
                                const command = Anchestor.commands.get(commandName);
                                if (!command)
                                        continue;
                ...
                async function handlerEvent() {
                        const { author } = event;
                        const allEventCommand = Anchestor.eventCommands.entries();
                        for (const [key] of allEventCommand) {
                                const getEvent = Anchestor.eventCommands.get(key);
                                if (!getEvent)
                                        continue;
                ...
                async function onEvent() {
                        const allOnEvent = Anchestor.onEvent || [];
                        const args = [];
                        const { author } = event;
                ...
                async function presence() {
                        try {
                                const fca = Anchestor.config.optionsFca || {};
